...
class NetFunnelHelper:
    ...
    # 네트워크 요청 타임아웃(초) 및 타임아웃/네트워크 오류 시 재시도 횟수
    REQUEST_TIMEOUT = 10
    MAX_RETRIES = 3
    RETRY_BACKOFF = 1  # 재시도 간 대기 시간(초)
    ...
    def _get(self, params: dict):
        """NetFunnel GET 요청. 타임아웃/네트워크 오류 시 자동으로 재시도합니다."""
        last_exc = None
        for attempt in range(1, self.MAX_RETRIES + 1):
            try:
                ...
                if attempt < self.MAX_RETRIES:
                    print(
                        f"[NetFunnel] 요청 실패({attempt}/{self.MAX_RETRIES}): "
                        f"{e.__class__.__name__} - 재시도합니다."
                    )
                    time.sleep(self.RETRY_BACKOFF)
        raise SRTNetFunnelError(last_exc) from last_exc
